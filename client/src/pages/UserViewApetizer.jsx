...
class UserViewApetizer extends react.Component {
  constructor(props) {
    super(props);
    this.state = {
      collapse: false,
  ...
  getAppetizerData = async () => {
    console.log("apet view");
  ...
  handleCart = async (e, Dishname, Category, Price) => {
    ...
    console.log("email", this.props.location.state.detail.email);
    console.log("Dishname", Dishname);
    console.log("Category", Category);
    console.log("Price", Price);
    await api.AddToCart(payload).then(res => {
      if (res.data.success) {
        window.alert(`item is inserted to cart successfully`);
      } else {
        window.alert(
          `items are not inserted into cart`
        );
      }
    });
...
export default UserViewApetizer;